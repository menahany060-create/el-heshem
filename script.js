...
// ===== INFINITE SLIDER - CSS ANIMATION =====
function initInfiniteSlider() {
  let slider = document.getElementById("slider");
  if (!slider) return;

  // Clone all cards للـ infinite loop
  let original = Array.from(slider.children);
  original.forEach(card => {
    let clone = card.cloneNode(true);
    slider.appendChild(clone);
  });

  // حساب العرض الكلي للـ original cards
  let totalWidth = original.reduce((sum, card) => {
    return sum + card.offsetWidth + 10; // 10 = gap
  }, 0);

  // inject CSS animation ديناميكي بناءً على العرض الفعلي
  ...
  style.innerHTML = `
    ...
    .slider-track {
      animation: infiniteScroll ${original.length * 2.5}s linear infinite;
    }
    .slider-track:hover {
  ...
  // حول الـ slider لـ track
  slider.classList.add("slider-track");

  // الأزرار - pause/resume بدل scroll
  document.querySelector(".slider-btn.left").onclick = function() {
    slider.style.animationDirection =
      slider.style.animationDirection === "reverse" ? "normal" : "reverse";
    setTimeout(() => {
      slider.style.animationDirection = "normal";
    }, 2000);
  };

  document.querySelector(".slider-btn.right").onclick = function() {
    // speed boost مؤقت
    slider.style.animationDuration = "3s";
    setTimeout(() => {
      slider.style.animationDuration = original.length * 2.5 + "s";
    }, 1000);
  };
}
